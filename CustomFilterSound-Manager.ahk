...
global iniFile := A_ScriptDir "\filters.ini"
global fileDir := ""
global fileArr := []
global folders := {}
global fileNames := ["1maybevaluable", "2currency", "3uniques", "4maps", "5highmaps", "6veryvaluable", "12leveling"]	; default/fallback, will get overwritten later
global folderList := ""
...
Loop Files, %workingDir%*.*, D
{
	...
	If (soundsFound) {
		folders[folderName] := {}
		folders[folderName].files := folder	
		folders[folderName].list := fileList		
		For key, val in fileNames {
			folders[folderName][val] := fileList
		}
...
Gui, Font, , Verdana
Gui, Add, Text, x10 y10 , Filter collection: "%workingDir%" 
Gui, Add, Button, gOpen1 x+10 yp-5, Open collection

Gui, Add, Text, x10 y+18 , Folder:
Gui, Add, DropDownList, vselectedFolder x200 yp-2 w250 gFolderChange, %folderList%

For key, fName in fileNames {
	yShift := key == 1 ? 20 : 10 
	Gui, Add, Text, x10 y+%yShift% , % fName
	Gui, Add, DropDownList, vsound%key% x200 yp-2 w250, % GetList(fName)
	; don't change the button names, the index number has to be there
	Gui, Add, Button, gplay x+10 yp-1, Play %key%
}


Gui, Add, Text, x10 y+20, Copy files to "%poeDir%"
Gui, Font, bold
Gui, Add, Button, gSave x10 y+5, Save selection
Gui, Add, Button, gCopy x10 y+5, Save and Copy selected files over
Gui, Font, cRed bold
Gui, Add, Text, x+10 yp+5, Files will be overwritten!
...
FolderChange:
	...
	newList := "|" folders[selectedFolder].list
	For key, fName in fileNames {
		GuiControl, , sound%key%, % "|" GetList(fName, selectedFolder)
	}	
Return

Save:
	Gui, Submit, NoHide
	If (StrLen(selectedFolder)) {		
		saveObj := {}
		For key, fName in fileNames {
			saveObj[fName] := sound%key%
		}
		SaveToIni(selectedFolder, saveObj)
	}
Return

SaveToIni(selected, obj) {
	file := A_ScriptDir "\filters.ini"
	
	savedSettings.general.selectedFilter := selected
	
	If (not savedSettings.HasKey(selected)) {
		savedSettings[selected] := obj
	}
	Else {
		For key, val in obj {
			If (StrLen(val)) {
				savedSettings[selected][key] := val
			}		
		}	
	}
	
	savedSettings.Save(file)
}

LoadIni() {
	dir := A_ScriptDir "\filters.ini"
	savedSettings := class_EasyIni(dir)
}
...
ApplySettings() {
	...
	For key, val in savedSettings[savedSettings.general.selectedFilter] {
		If (StrLen(val)) {
			regFile := RegExReplace(val, "i)(\+|\.|\?|\$|\\|\!|\[|\]|\(|\))", "\$1")
			folders[savedSettings.general.selectedFilter][key] := RegExReplace(folders[savedSettings.general.selectedFilter].list, "i)(" regFile ")(\||$)", "$1$2|")
		}	
	}
	
	For key, val in savedSettings {
		For k, v in val {
			If (StrLen(v)) {
				regFile := RegExReplace(v, "i)(\+|\.|\?|\$|\\|\!|\[|\]|\(|\))", "\$1")
				folders[key][k] := RegExReplace(folders[key].list, "i)(" regFile ")(\||$)", "$1$2|")
			}
		}			
	}
}
...
ReadFileNames(list) {
	l := []	
	Loop, Read, %A_ScriptDir%\fileNameList.txt
	{
		; This loop retrieves each line from the file, one at a time.
		line := A_LoopReadLine
		If (not RegExMatch(line,"i)^;")) {
			l.push(line)
		}
	}
...
GetList(id, f = "") {
	f := (StrLen(f)) ? f : savedSettings.general.selectedFilter

	If (StrLen(folders[f].list)) {		
		Return folders[f][id]
	}
	Else {
		Return ""
...
play:	
	Gui, Submit, NoHide
	GuiControlGet, playButtonText, FocusV	
	soundIndex := Trim(RegExReplace(playButtonText, "i)Play.*(\d+).*", "$1"))
	playSound(sound%soundIndex%, workingDir . selectedFolder)
Return


open1:
	Run, explore %workingDir%
Return
